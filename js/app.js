...
function pintarTabla(infoSolicitud,infoPropuesta,nombreCopropiedad,productosIniciales){
    ...
    let subTotalesProveedores= infoPropuesta.map(proveedor=>proveedor.productosProveedores)
        ...
        let subtotalesProveedoresAPintar= subTotalesProveedores.map(infoSubtotalesProveedor=> pintarSubtotal("$ "+infoSubtotalesProveedor.subtotalTotal, "$ "+infoSubtotalesProveedor.subtotalTotal))
                                            .reduce(((acc, productoString)=> acc+productoString), "");

        let subtotalesIVA=subTotalesProveedores.map((infoSubtotalesProveedor, index)=> pintarIVA("%"+infoPropuesta[index].ivaProducto, "$ "+Math.trunc(infoSubtotalesProveedor.subtotalTotal*(19/100))))
                                    .reduce(((acc, productoString)=> acc+productoString), "");

        let granTotales=subTotalesProveedores.map((infoSubtotalesProveedor, index)=> pintarTotal( "$ "+(Math.trunc(infoSubtotalesProveedor.subtotalTotal*(19/100))+infoSubtotalesProveedor.subtotalTotal)))
...
function pintarIVA(primerParametro, segundoParametro){
    return '<td colspan="1" class="cantidadProducto costos" id="cantidadProducto"></td>'+primerParametro+'<td class="cantidadProducto costos" id="cantidadProducto">'+segundoParametro+'</td>';
}
